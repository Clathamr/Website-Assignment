...
class DisplayStudentForm(FlaskForm):
    submit = SubmitField('Display the students')


class DeactivateStudentForm(FlaskForm):
    student_id = StringField('Student ID to deactivate', validators=[DataRequired()])
    submit = SubmitField('Deactivate the student')

    def validate_student_id(self, student_id):
        studentlist = [student_id] #This is only to allow my second if to run
        if not student_id.data:
            raise ValidationError('This student is not active, cannot deactivate.')
        if student_id.data not in studentlist: # ran out of time, student list should be a list of student objects
